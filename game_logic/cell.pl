:- module(cell, [add_cell/5, remove_cell/5, is_piled_up/4,
                 ...
                 queen_on_field/1,get_cells_list/1,is_mosquito/1,empty_cell/1,
                 top_of_pile/1]).
...
piledUp_update_leave(RO,CO) :-
    findall(cell(Bug,Color,PiledUp,RO,CO),get_cell(cell(Bug,Color,PiledUp,RO,CO)),Cells),
    decrease_piledUp(Cells).

piledUp_update_arrival(RD,CD) :-
    findall(cell(Bug,Color,PiledUp,RD,CD),get_cell(cell(Bug,Color,PiledUp,RD,CD)),Cells),
    increase_piledUp(Cells).
